(function () {
    ...
    angular
        ...
        .component("sdSearchSubjectsMap", {
            template: "<div id='map'></div>",
            controller: SearchSubjectsMapController,
            bindings: {
                zoom: "@"
            }
        });

    SearchSubjectsMapController.$inject = ["$scope", "$q", "$element",
    ...
    function SearchSubjectsMapController($scope, $q, $element, currentOrigin, myLocation, Map, searchSubjects,
                                         APP_CONFIG) {

        var vm = this;

        vm.$onInit = function () {
            console.log("SearchSubjectsMapController", $scope);
        }
        ...
        vm.$postLink = function () {
            ...
            $scope.$watch(
                function () {
                    return searchSubjects.getThingImages();
                },
                function () {
                    var thing_images = searchSubjects.getThingImages();
                    if (angular.isArray(thing_images)) {
                        console.log("Thing Images", thing_images);
                        vm.images = thing_images;
                        displaySubjects();
                    }
                });
        ...
        function displaySubjects() {
            ...
            var priorityThingImage;
            angular.forEach(vm.images, function (ti) {
                if (ti.position != null) {
                    if (ti.priority === 0) {
                        priorityThingImage = ti;
                    } else {
                        displaySubject(ti);
                    }
                }
            ...
            if (priorityThingImage != null) {
                displaySubject(priorityThingImage);
            }

            // This is a fix for a known map issue
            var cnt = vm.map.map.getCenter();
            cnt.e += 0.000001;
            vm.map.map.panTo(cnt);
            cnt.e -= 0.000001;
            vm.map.map.panTo(cnt);
        }
    ...
    SearchSubjectsMapController.prototype.updateOrigin = function () {
    ...
    SearchSubjectsMapController.prototype.setActiveMarker = function (thing_id, image_id) {
        if (!this.map) {
            return;
    ...
    SearchSubjectsMapController.prototype.originInfoWindow = function (location) {
    ...
    SearchSubjectsMapController.prototype.thingInfoWindow = function (ti) {
