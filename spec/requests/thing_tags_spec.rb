...
RSpec.describe "ThingTags", type: :request do
  ...
  shared_examples "cannot get links" do |status|
    it do
      jget thing_thing_tags_path(linked_thing_id)
      expect(response).to have_http_status(status)
    end
  end
  shared_examples "cannot get linkables" do |status|
    it do
      jget tag_linkable_things_path(linked_tag_ids[0])
      #pp parsed_body
      expect(response).to have_http_status(status)
  ...
  shared_examples "cannot delete link" do |status|
    it do
      jdelete thing_thing_tag_path(linked_thing_id, linked_tag_id)
      expect(response).to have_http_status(status)
    end
  end
  ...
  describe "ThingTag Authn policies" do
    ...
    context "user is anonymous" do
      before(:each) { logout }
      it_should_behave_like "cannot get links", :forbidden
      it_should_behave_like "cannot get linkables", :forbidden
